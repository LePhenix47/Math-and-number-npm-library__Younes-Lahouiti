...
exports.solveQuadratic = exports.solveLinear = exports.isPrime = exports.factorial = exports.logarithm = exports.nthRoot = void 0;
/**
...
function nthRoot(value, root) {
    ...
    var rootIsInvalid = root === 0;
    if (rootIsInvalid) {
        throw new Error("The root cannot be null, as it returns an exponent with a division by 0");
    }
    // We check that the value is negative AND that the root is even
    var valueOfRootIsInvalid = value < 0 && root % 2 === 0;
    if (valueOfRootIsInvalid) {
        // Negative values cannot have an even root
        //∛(-27) = -3 but √(-16) = undefined
        throw new Error("The root: ".concat(root, " of the value: ").concat(value, " passed is invalid, cannot have a negative value with an even root"));
    }
    ...
    var calculatedRoot = Math.pow(Math.abs(value), (1 / root));
...
function logarithm(value, base) {
    ...
    var baseIsInvalid = base <= 0 || base === 1;
    if (baseIsInvalid) {
        throw new Error("The base of the logarithm ".concat(base <= 0 ? "is negative or null" : "returns a division by 0"));
    }
    //Logarithmic functions cannot have a negative or null value
    var valueIsInvalid = value <= 0;
    if (valueIsInvalid) {
...
function factorial(number) {
    ...
    var argumentIsNotANumber = isNaN(number);
    if (argumentIsNotANumber) {
        throw new Error("Argument passed: ".concat(number, " is not a number"));
    }
    //This function is the conventional factorial,
    //thus negative and floating numbers do not have a factorial
    var numberHasNoFactorial = !Number.isInteger(number) || number < 0;
    if (numberHasNoFactorial) {
        throw new Error("Number passed: ".concat(number, " cannot have a factorial"));
    }
    //1! = 1 and 0! = 1, no need to compute them
    var hasOneAsValue = number === 0 || number === 1;
    if (hasOneAsValue) {
        return 1;
    }
...
function isPrime(number) {
    ...
    var numberIsNotPrime = !Number.isInteger(number) ||
        number < 2 ||
        (number !== 2 && number % 2 === 0);
    if (numberIsNotPrime) {
        return false;
    ...
    var isSimplePrime = number === 2;
    if (isSimplePrime) {
        return true;
    }
    for (var i = 2; i < number; i++) {
        var isDivisibleByNumber = number % i === 0;
...
function solveLinear(a, b) {
    //Error handling logic here
    var argumentsAreInvalid = isNaN(a) || isNaN(b);
    if (argumentsAreInvalid) {
        throw new Error("Arguments are invalid, expected a number but instead got: ".concat(typeof a, " for a and ").concat(typeof b, " for b"));
    }
...
function solveQuadratic(a, b, c) {
    //Error handling logic here:
    var argumentsAreInvalid = isNaN(a) || isNaN(b) || isNaN(c);
    if (argumentsAreInvalid) {
        throw new Error("Arguments are invalid, expected a number but instead got: ".concat(typeof a, " for a, ").concat(typeof b, " for b and ").concat(typeof c, " for c "));
    }
    //We compute Δ = b² - 4ac
    var delta = computeDiscriminant(a, b, c);
    /**
    ...
    var hasOneRealSolution = delta === 0;
